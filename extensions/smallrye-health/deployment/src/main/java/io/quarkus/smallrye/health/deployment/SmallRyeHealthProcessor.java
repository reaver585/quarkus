...
class SmallRyeHealthProcessor {
    ...
    void registerHealthUiHandler(
        ...
        if (shouldInclude(launchMode, healthConfig)) {
            ...
            var sortedWebRootConfigurations = result.getWebRootConfigurations().stream()
                    .sorted(
                            Comparator
                                    .comparing((FileSystemStaticHandler.StaticWebRootConfiguration wrc) -> wrc.getFileSystem())
                                    .thenComparing(wrc -> wrc.getFileSystem()))
                    .toList();

            Handler<RoutingContext> handler = recorder.uiHandler(result.getFinalDestination(),
                    healthUiPath, sortedWebRootConfigurations, shutdownContext);
