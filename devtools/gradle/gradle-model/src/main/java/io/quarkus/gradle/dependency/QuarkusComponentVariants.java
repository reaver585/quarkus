...
public class QuarkusComponentVariants {
    ...
    private ConditionalDependency newConditionalDep(Dependency dep) {
        final Configuration config = project.getConfigurations().detachedConfiguration(dep).setTransitive(true);
        ...
        for (var a : config.getResolvedConfiguration().getResolvedArtifacts()) {
            System.out.println("[pupa]: Resolved artifact for " + dep + ": " + a.getId().getComponentIdentifier());
            resolvedArtifact = a;
            break;
        }
